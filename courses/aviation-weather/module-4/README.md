# Aviation Forecast Products & Warnings — Moodle Upload Instructions
...
| File | What it is |
|------|------------|
| `*-module-4.mbz` | **Restore this in Moodle for the one-upload path** (only present if you ran `build_mbz.py --module`). A Moodle backup of this module — overview Page → lesson Page → Quiz per lesson, fully wired. |
...
3. Name it `Aviation Forecast Products & Warnings`.
...
3. **Import category:** create or pick `Aviation Forecast Products & Warnings Questions`.
4. Drag `module_quiz_all_questions.xml` into the upload box → **Import** → **Continue**.
5. Add a **Quiz** activity to the module's section, name it `Aviation Forecast Products & Warnings Certification Quiz`, and under **Edit quiz → Add → from question bank**, select all the imported questions.
